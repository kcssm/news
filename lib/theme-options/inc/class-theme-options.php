...
class GPPThemeOptions {
	...
	public function addPages() {
		...
		$admin_page = add_theme_page( __( 'Theme Options', '_gpp' ), __( 'Theme Options', '_gpp' ), 'manage_options', 'gpp-options', array( &$this, 'displayPage' ) );
	...
	public function createSetting( $args = array() ) {
		...
		$defaults = array(
			'id'      => 'default_field',
			'title'   => __( 'Default Field', '_gpp' ),
			'desc'    => __( 'This is a default description.', '_gpp' ),
			'std'     => '',
			'type'    => 'text',
			'section' => 'general',
			'choices' => array(),
		...
		add_settings_field( $id, $title, array( $this, 'buildOptions' ), 'gpp-options', $section, $field_args );
	}

	public function updateNotice( $notice = null, $message = null ) {

		if ( is_null( $message ) ) {
			$message = __( 'Setting updated', '_gpp' );
		}
		...
		$classes = __( 'updated ', '_gpp' );

	    if ( $notice ) {
			add_settings_error( 'gpp-notices', 'gpp-updated', $message, $classes );
	    }

		settings_errors( 'gpp-notices' );
	}
	...
	public function displayPage() {
		...
		echo '<div id="gpp-wrap" class="wrap">';
		screen_icon( 'themes' );
		echo '<h2>' . __( 'Theme Options', '_gpp' ) . '</h2>';
		echo '<ul class="theme-info">';
		echo '<li><a href="' . $this->theme[ 'uri' ] . '" target="_blank">' . $this->theme[ 'name' ] . '</a></li>';
		echo '<li>' . __( 'Version: ', '_gpp' ) . $this->theme['version'] . '</li>';
		echo '<li><a href="' . $this->theme[ 'support' ] . '"" target="_blank">' . __( 'Support', '_gpp' ) . '</a></li>';
		echo '<li><a class="thickbox" href="' . get_option( 'siteurl' ) . '/wp-admin/admin-ajax.php?action=readme&height=600&width=640" title="' . __( 'Theme Instructions', '_gpp' ) . '">' . __( 'Instructions', '_gpp' ) . '</a></li>';
		echo '</ul>';

		echo '<form action="options.php" method="post" id="gpp_form" enctype="multipart/form-data">';

		$this->updateNotice();

		settings_fields( '_gpp_options' );
		...
		if ( $this->tabbed == true ) {
			...
			print '<div id="gpp-tabs" class="ui-tabs">';
		...
		echo '<p class="submit"><input name="Submit" type="submit" class="button-primary" value="' . __( 'Save Changes', '_gpp' ) . '" /></p>
		<p class="reset"><input type="button" class="reset-button reset-handle button-secondary" name="reset" value="' . __( 'Restore Defaults', '_gpp' ) . '" /></p>
	</form>';
	...
	public function buildOptions( $args = array() ) {
		...
		switch ( $type ) {
			...
			case 'checkbox':
				echo '<input class="checkbox' . $field_class . '" type="checkbox" id="' . $id . '" name="_gpp_options[' . $id . ']" value="1" ' . checked( $theme_options[ $id ], 1, false ) . ' /> <label for="' . $id . '">' . $desc . '</label>';
				break;

			case 'select':
				echo '<select class="select' . $field_class . '" name="_gpp_options[' . $id . ']">';
			...
			case 'multiselect':
				echo '<select class="select' . $field_class . '" name="_gpp_options[' . $id . '][]" multiple="multiple">';
				foreach ( $choices as $value => $label ) {
			...
			case 'multicheck':
				...
				foreach ( $choices as $value => $label ) {
					...
					echo '<input class="checkbox' . $field_class . '" type="checkbox" name="_gpp_options[' . $id . '][]" id="' . $id . $i . '" value="' . esc_attr( $value ) . '"' . $selected . '> <label for="' . $id . $i . '">' . $label . '</label>';
					echo '<br />';
					$i++;
				}
				if ( $desc != '' )
			...
			case 'radio':
				$i = 0;
				foreach ( $choices as $value => $label ) {
					echo '<input class="radio' . $field_class . '" type="radio" name="_gpp_options[' . $id . ']" id="' . $id . $i . '" value="' . esc_attr( $value ) . '" ' . checked( $theme_options[ $id ], $value, false ) . '> <label for="' . $id . $i . '">' . $label . '</label>';
			...
			case 'radio_img':
				...
				foreach( $choices as $k => $v ) {
					$selected = ( checked( $theme_options[ $id ], $k, false ) != '' )?' gpp-radio-img-selected':'';
					echo '<label class="gpp-radio-img'.$selected.' gpp-radio-img-'.$theme_options[ $id ].'" for="' . $id . '_' . $i . '">';
					echo '<input type="radio" id="' . $id . '_' . $i . '" name="_gpp_options[' . $id . ']" '.$field_class.' value="'.$k.'" '.checked( $theme_options[ $id ], $k, false ).'/>';
					echo '<img src="'.$v[ 'img' ].'" alt="'.$v[ 'title' ].'" onclick="jQuery:gpp_radio_img_select(\'' . $id . '_' . $i . '\', \''.$theme_options[ $id ].'\');" />';
			...
			case 'textarea':
				echo '<textarea class="' . $field_class . '" id="' . $id . '" name="_gpp_options[' . $id . ']" placeholder="' . $std . '" rows="5" cols="30">' . esc_textarea( $theme_options[ $id ] ) . '</textarea>';
				if ( $desc != '' )
					echo '<span class="description">' . $desc . '</span>';
				break;

			case 'password':
				echo '<input class="regular-text' . $field_class . '" type="password" id="' . $id . '" name="_gpp_options[' . $id . ']" value="' . esc_attr( $theme_options[ $id ] ) . '" autocomplete="off" />';
			...
			case 'upload':
				echo '<input id="' . $id . '" class="upload-url' . $field_class . '" type="hidden" name="_gpp_options[' . $id . ']" value="' . esc_attr( $theme_options[ $id ] ) . '" /><div class="imgpreview">';

				if( $theme_options[ $id ] != '' ) {
					echo '<img class="gpp-screenshot" id="gpp-screenshot-'.$id.'" src="' . esc_attr( $theme_options[ $id ] ) . '" />';
				}
				echo "</div>";
				...
				echo '<a href="javascript:void(0);" class="upload_button button-secondary"' . $upload . ' rel-id="' . $id . '">' . __('Browse', '_gpp') . '</a>';
				echo '<a href="javascript:void(0);" class="upload_button_remove"' . $remove . ' rel-id="' . $id . '">' . __('Remove Upload', '_gpp') . '</a>';
			...
			case 'color':
				...
				echo '<input class="popup-colorpicker' . $field_class . '" type="text" id="' . $id . '" name="_gpp_options[' . $id . ']" placeholder="' . $std . '" value="' . esc_attr( $theme_options[ $id ] ) . '" />
			...
			case 'slide':
				...
				if ( $theme_options[ $id ] <> '' ) {
					...
					foreach ( $slides as $slide ) {
						...
						echo '<li class="slide">';
						echo '<span class="description">' . __( 'Slide Title', '_gpp' ) . '</span>';
						echo '<input class="regular-text' . $field_class . '" name="_gpp_options[' . $id . '][title][]" placeholder="' . $std . '" id="' . $id . '_title_' . $i . '"  value="' . $slide[ 'title' ] . '" type="text" />';

						echo '<span class="description">' . __( 'Slide Link', '_gpp' ) . '</span>';
						echo '<input class="regular-text' . $field_class . '" name="_gpp_options[' . $id . '][link][]" placeholder="' . $std . '" id="' . $id . '_title_' . $i . '"  value="' . $slide[ 'link' ] . '" type="text" />';

						echo '<span class="description">' . __( 'Slide Caption', '_gpp' ) . '</span>';
						echo '<textarea class="' . $field_class . '" name="_gpp_options[' . $id . '][caption][]" id="' . $id . '_caption_' . $i . '" cols="40" rows="4">' . $slide[ 'caption' ] . '</textarea>';

						echo '<span class="description">' . __( 'Slide Image', '_gpp' ) . '</span>';
						echo '<input class="upload-input-text src" name="_gpp_options[' . $id . '][image][]" id="' . $id . '_image_' . $i . '" type="text" value="'. $slide['image'] . '" type="text" />
						<a href="' . get_option( 'siteurl' ) . '/wp-admin/admin-ajax.php?action=choice&width=150&height=100" id="' . $id . '_button" class="button upbutton">' . __( 'Upload', '_gpp' ) . '</a>';
		                ...
		                echo '</div>';
						echo '<a class="remove_slide submitdelete">' . __( 'Delete Slide', '_gpp' ) . '</a>';
						echo '</li>';
						$i++;
					}
					...
					echo '<li class="slide">';
					echo '<span class="description">' . __( 'Slide Title', '_gpp' ) . '</span>';
					echo '<input class="regular-text' . $field_class . '" name="_gpp_options[' . $id . '][title][]" placeholder="' . $std . '" id="'. $id .'_title_' . $i . '"  value="" type="text" />';

					echo '<span class="description">' . __( 'Slide Link', '_gpp' ) . '</span>';
					echo '<input class="regular-text' . $field_class . '" name="_gpp_options[' . $id . '][link][]" placeholder="' . $std . '" id="'. $id .'_title_' . $i . '"  value="" type="text" />';

					echo '<span class="description">' . __( 'Slide Caption', '_gpp' ) . '</span>';
					echo '<textarea class="' . $field_class . '" name="_gpp_options[' . $id . '][caption][]" id="' . $id . '_caption_' . $i . '" cols="40" rows="4"></textarea>';

					echo '<span class="description">' . __( 'Slide Image', '_gpp' ) . '</span>';
					echo '<input class="upload-input-text src" name="_gpp_options[' . $id . '][image][]" id="' . $id . '_image_' . $i . '" type="text" value="" type="text" />
					<a href="' . get_option('siteurl') . '/wp-admin/admin-ajax.php?action=choice&width=150&height=100" id="' . $id . '_button" class="button upbutton">' . __( 'Upload', '_gpp' ) . '</a>';
			...
			case 'text':
				default:
		 		echo '<input class="regular-text' . $field_class . '" type="text" id="' . $id . '" name="_gpp_options[' . $id . ']" placeholder="' . $std . '" value="' . esc_attr( $theme_options[ $id ] ) . '" autocomplete="off" />';
		 		if ( $desc != '' )
		 			echo '<span class="description">' . $desc . '</span>';
		 		break;

		 	case 'file':
		 		echo '<input type="file" name="_gpp_options_file" />';
		 		echo '<input type="submit" name="filesubmit" class="button" value="Upload">';
		 		echo '<br /><br />';

		 		if ( isset( $theme_options[ '_gpp_options_file' ][ 'attachment_id' ] ) ){
		 			echo wp_get_attachment_image( $theme_options[ '_gpp_options_file' ][ 'attachment_id' ], 'medium' );
		 			echo '<br /><br /><input type="submit" name="remove_file_upload" id="remove_file_upload" class="button" value="Remove Image">';
		 		}
	...
	public function initializeSettings( $settings=null ) {
		...
		update_option( '_gpp_options', $default_settings );
	}
	...
	*/
	public function registerSettings() {
		...
		register_setting( '_gpp_options', '_gpp_options', array ( &$this, 'formHandler' ) );

		foreach ( $this->sections as $slug => $title ) {
			add_settings_section( $slug, $title, array( &$this, 'displaySection' ), 'gpp-options' );
		}
	...
	*/
	public function addScripts() {
		...
		wp_register_script( 'gpp-options-scripts', get_template_directory_uri() . '/lib/theme-options/js/options.js', array( 'jquery', 'media-upload','thickbox', 'farbtastic' ) );
		wp_enqueue_script( 'gpp-options-scripts' );
	...
	*/
	public function addStyles() {
		...
		wp_register_style( 'gpp-options-styles', get_template_directory_uri() . '/lib/theme-options/css/options.css' );
		if ( $this->tabbed == true )
			wp_enqueue_style( 'gpp-options-styles' );
	...
	public function formHandler( $input ) {
		...
		if ( ! isset( $input[ 'reset_theme' ] ) ) {
			...
			if ( $_FILES[ '_gpp_options_file' ][ 'size' ] > 0 ) {
				// die('we have file');
				$media = new GPPMediaUpload;
				$input[ '_gpp_options_file' ] = $media->saveUpload( $field_name='_gpp_options_file' );
			} else {
				// die('we have NO file');
				$input[ '_gpp_options_file' ] = $theme_options[ '_gpp_options_file' ];
			}

			if ( isset( $_POST[ 'remove_file_upload' ] ) ) {
				unset( $input[ '_gpp_options_file' ] );
			}
